...
>
> **v6** — Mejoras de calidad de edición a partir de feedback sobre el primer video real: audio limpio (denoise + normalización vía ffmpeg, `common/audio_cleanup.py`), transiciones con crossfade entre segmentos de B-roll en vez de cortes secos, los clips más cortos que su porción ahora hacen loop en "ida y vuelta" (boomerang) en vez de reiniciar de golpe, subtítulos con una fuente real que sí soporta tildes/ñ (`assets/fonts/BigShoulders-Bold.ttf`, licencia OFL) más una caja semitransparente detrás para legibilidad, overlays opcionales (memes/avatar/stickers, `assets/overlays_usuario/`, endpoints `/api/overlays/*`) flotando en la esquina superior derecha (a propósito lejos de los subtítulos), y música de fondo opcional (`assets/musica/`, el usuario deja sus propias pistas royalty-free). Ver §12.
>
> **v7** — Generación de imágenes por IA (`common/imagen_ia.py`, Pollinations.ai, gratis y sin API key) para cuando el banco de stock no tiene nada relevante para el tema (típicamente personajes/videojuegos puntuales, como pasó con un video sobre Mario Kart que salió con B-roll de redes neuronales sin nada de videojuegos). Dos usos: (1) `broll.ratio_ia` mezcla clips generados por IA con el banco de stock, y además la IA rescata cualquier palabra clave que el banco no encuentre; (2) overlays generados a pedido con un prompt de texto (`/api/overlays/generar_ia`, botón "Generar con IA" en la webapp), que se suman a la cola igual que un overlay subido a mano. Ver §13.
...
## 12. Mejoras de edición (v6)

A partir de revisar el primer video real generado, se identificaron varios puntos de calidad de edición bastante básicos que faltaban. Todos viven en `scripts/04_assemble_video.py` salvo el de audio:

- **Audio limpio** (`common/audio_cleanup.py`): antes de transcribir y de usar el audio en el video, se corre por un filtro de ffmpeg (`highpass` + `afftdn` + `loudnorm`) que saca ruido de fondo y normaliza el volumen a -16 LUFS. Configurable con `audio.limpiar` (default `true`); si ffmpeg falla por algún motivo, sigue con el audio original en vez de cortar el pipeline.
- **Transiciones con crossfade** entre segmentos de B-roll, en vez del corte seco de antes. Duración configurable (`video.transicion_seg`, default 0.4s; poner en `0` para volver a cortes secos).
- **Loop "boomerang"**: cuando un clip de B-roll dura menos que la porción que le toca, en vez de reiniciar desde el frame 0 (salto visible) ahora se reproduce ida y vuelta (el final del loop es casi idéntico al principio, sin costura).
- **Subtítulos con fuente real**: la fuente default de MoviePy no renderizaba bien tildes ni la "ñ" (se veían como un cuadrado/carácter roto). Se bundleó `assets/fonts/BigShoulders-Bold.ttf` (Google Fonts, licencia SIL OFL 1.1, incluida en el repo) como default, más una caja semitransparente detrás del texto para que se lea sobre cualquier fondo. Configurable (`video.fuente`, `video.tam_fuente`, `video.subtitulo_fondo_opacidad`, `video.subtitulo_pos_y`).
- **Overlays** (memes/avatar/stickers, `common/overlays.py`, `assets/overlays_usuario/`, endpoints `/api/overlays/*`): igual mecánica que "media propia" (queda pendiente hasta el próximo video, se consume una sola vez), pero en vez de reemplazar B-roll a pantalla completa, aparece flotando en la **esquina superior derecha** por unos segundos — a propósito arriba y no abajo, para no taparse con los subtítulos (un bug real que apareció durante las pruebas: al principio los overlays iban abajo a la derecha y tapaban parte del texto).
- **Música de fondo opcional** (`common/musica.py`, `assets/musica/`): el proyecto no incluye pistas (son archivos con su propia licencia); si el usuario deja las suyas ahí, se elige una al azar, se loopea/recorta a la duración del video, y se mezcla por debajo de la narración (`musica.activar`, `musica.volumen`).

## 13. Generación de imágenes por IA (v7)
